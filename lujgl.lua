...
-- @param hints (Optional) Table of values to be passed to glfwOpenWindowHint. Defaults to {glfwconst.GLFW_RESIZABLE=true,glfwconst.GLFW_VISIBLE=true, glfwconst.GLFW_DECORATED=true}
function LuJGL.initialize(name, w, h, hints)
	...
	if hints then
		for k,v in pairs(hints) do glfw.glfwWindowHint(k,v) end
	else
		glfw.glfwWindowHint(glfwconst.GLFW_RESIZABLE, true)
		glfw.glfwWindowHint(glfwconst.GLFW_VISIBLE, true)
		glfw.glfwWindowHint(glfwconst.GLFW_DECORATED, true)
	end
	
	glfw.glfwWindowHint(glfwconst.GLFW_RED_BITS, 8)
	glfw.glfwWindowHint(glfwconst.GLFW_GREEN_BITS, 8)
	glfw.glfwWindowHint(glfwconst.GLFW_BLUE_BITS, 8)
	glfw.glfwWindowHint(glfwconst.GLFW_ALPHA_BITS, 8)
	glfw.glfwWindowHint(glfwconst.GLFW_DEPTH_BITS, 24)
	glfw.glfwWindowHint(glfwconst.GLFW_STENCIL_BITS, 8)
	glfw.glfwWindowHint(glfwconst.GLFW_CONTEXT_VERSION_MAJOR, 2)
	glfw.glfwWindowHint(glfwconst.GLFW_CONTEXT_VERSION_MINOR, 1)
	
	LuJGL.window = glfw.glfwCreateWindow(w,h,name,nil,nil)

	if LuJGL.window == nil then
		glfw.glfwTerminate()
		error("error initializing glfw window",0)
	end
	
	glfw.glfwMakeContextCurrent(LuJGL.window)
	
	local size_buffer = ffi.new("int[2]")
	glfw.glfwGetWindowSize(LuJGL.window, size_buffer, size_buffer + 1)
	...
	local last_wheel_posx = 0
	local last_wheel_posy = 0
	glfw.glfwSetScrollCallback(LuJGL.window, create_callback(function(window, posx, posy)
		call_callback(event_cb, "wheel", posx - last_wheel_posx, posy - last_wheel_posy)
		last_wheel_posx = posx
		last_wheel_posy = posy
	end))
end

function LuJGL.deinitialize()
	glfw.glfwDestroyWindow(LuJGL.window)
	glfw.glfwTerminate()
end
...
function LuJGL.checkError()
	...
	if errcode ~= glconst.GL_NO_ERROR then
...
if LuJGL.stb_image then
	...
	function LuJGL.loadTexture(filepath, fchannels, mipmaps, wrap)
		...
		gl.glBindTexture(glconst.GL_TEXTURE_2D,texid)
		
		if wrap then
			gl.glTexParameterf(glconst.GL_TEXTURE_2D, glconst.GL_TEXTURE_WRAP_S, glconst.GL_REPEAT)
			gl.glTexParameterf(glconst.GL_TEXTURE_2D, glconst.GL_TEXTURE_WRAP_T, glconst.GL_REPEAT)
		end
		
		if mipmaps then
			gl.glTexParameterf(glconst.GL_TEXTURE_2D, glconst.GL_TEXTURE_MIN_FILTER, glconst.GL_LINEAR_MIPMAP_NEAREST)
			gl.glTexParameterf(glconst.GL_TEXTURE_2D, glconst.GL_TEXTURE_MAG_FILTER, glconst.GL_LINEAR)
			local ok = glu.gluBuild2DMipmaps(glconst.GL_TEXTURE_2D, fchannels or imgdatabuffer[2], imgdatabuffer[0], imgdatabuffer[1],
				tex_channels2glconst[fchannels or imgdatabuffer[2]], glconst.GL_UNSIGNED_BYTE, image)
			if ok ~= 0 then
				error(ffi.string(glu.gluErrorString(ok)))
			end
		else
			gl.glTexParameterf(glconst.GL_TEXTURE_2D, glconst.GL_TEXTURE_MIN_FILTER, glconst.GL_LINEAR)
			gl.glTexParameterf(glconst.GL_TEXTURE_2D, glconst.GL_TEXTURE_MAG_FILTER, glconst.GL_LINEAR)
			gl.glTexImage2D(glconst.GL_TEXTURE_2D, 0, fchannels or imgdatabuffer[2], imgdatabuffer[0], imgdatabuffer[1],
				0, tex_channels2glconst[fchannels or imgdatabuffer[2]], glconst.GL_UNSIGNED_BYTE, image)
		end
...
glconst = LuJGL.glconst

tex_channels2glconst = {
	[1] = glconst.GL_ALPHA,
	[2] = glconst.GL_LUMINANCE_ALPHA,
	[3] = glconst.GL_RGB,
	[4] = glconst.GL_RGBA,
}
